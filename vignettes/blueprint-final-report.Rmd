...
epidemic_summary_outputs_0.4_random <- epidemic_summary_outputs[epidemic_summary_outputs$coverage_type == "none", ]
epidemic_summary_outputs_0.6_random <- epidemic_summary_outputs_0.4_random
epidemic_summary_outputs_0.8_random <- epidemic_summary_outputs_0.4_random
epidemic_summary_outputs_0.4_targeted <- epidemic_summary_outputs_0.4_random
epidemic_summary_outputs_0.6_targeted <- epidemic_summary_outputs_0.6_random
epidemic_summary_outputs_0.8_targeted <- epidemic_summary_outputs_0.8_random

epidemic_summary_outputs_0.4_random$efficacy <- 0.4
epidemic_summary_outputs_0.6_random$efficacy <- 0.6
epidemic_summary_outputs_0.8_random$efficacy <- 0.8
epidemic_summary_outputs_0.4_targeted$efficacy <- 0.4
epidemic_summary_outputs_0.6_targeted$efficacy <- 0.6
epidemic_summary_outputs_0.8_targeted$efficacy <- 0.8

epidemic_summary_outputs_0.4_random$coverage_type <- rep("random", 2)
epidemic_summary_outputs_0.6_random$coverage_type <- rep("random", 2)
epidemic_summary_outputs_0.8_random$coverage_type <- rep("random", 2)
epidemic_summary_outputs_0.4_targeted$coverage_type <- rep("targeted_riskiness", 2)
epidemic_summary_outputs_0.6_targeted$coverage_type <- rep("targeted_riskiness", 2)
epidemic_summary_outputs_0.8_targeted$coverage_type <- rep("targeted_riskiness", 2)

epidemic_summary_outputs2 <- rbind(epidemic_summary_outputs, 
                                  epidemic_summary_outputs_0.4_targeted,
                                  epidemic_summary_outputs_0.6_targeted,
                                  epidemic_summary_outputs_0.8_targeted,
                                  epidemic_summary_outputs_0.4_random,
                                  epidemic_summary_outputs_0.6_random,
                                  epidemic_summary_outputs_0.8_random) %>%
  filter(coverage_type != "none")

epidemic_summary_outputs2 |>
...
rel_impact <- ggplot(subset(x, coverage <= 0.7), aes(x = coverage, y = inc_reduction_rel, col = coverage_type)) +
  ...
  scale_colour_manual(values = blueprint_colours, labels = c("Random", "Targeted\nRiskiness")) +
  theme_bw() +
  labs(x = "Coverage", y = "Fold-increase impact relative to 10% coverage", colour = "Coverage\nType")
...
ggplot(data = df_comparison, aes(x = 100 * hypothetical_reduction, 
  ...
  facet_grid(. ~ archetype,
             ...
             labeller = as_labeller(c(`flu` = "Influenza",
                                      `sars_cov_2` = "SARS-CoV-2"))) +
  scale_colour_manual(values = c("#00AFFF", "#03113E"), labels = c("Random", "Targeted")) + 
...
``` {r comparison-2, fig.cap = "(ref:comparison-2)"}

a <- ggplot(data = filter(df_comparison, coverage_type == "random"), 
            aes(x = 100 *(1- hypothetical_reduction), 
                y = 100 * incidence_percentage_reduction_mean / (100 * hypothetical_reduction), 
                col = 100 * hypothetical_reduction)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = 100 * incidence_percentage_reduction_lower / (100 * hypothetical_reduction),
                    ...
                    col = 100 * hypothetical_reduction), linewidth = 1) +
  facet_grid(. ~ archetype, scales = "free_y",
             labeller = as_labeller(c(`flu` = "Influenza",
                                      `sars_cov_2` = "SARS-CoV-2"))) +
  coord_cartesian(xlim = c(60, 100)) +
  labs(x = "% of Transmission Remaining Predicted By Simple Model\n(100% - % Reduction Predicted by Simple Model)",
       y = "Epidemiological Value\nof Averted Infection",
       col = "% Reduction\nin incidence\npredicted by\nsimple model") +
  theme_bw()
