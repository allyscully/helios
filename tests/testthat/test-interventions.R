...
test_that("set_uvc() correctly assigns Far-UVC parameters for all settings and coverage types", {
  ...
  expect_identical(
    object = parameters_workplace,
    expected = test_parameters_workplace
  )
  ...
  # Check that the two parameter lists are identical:
  expect_identical(
    object = parameters_school,
    expected = test_parameters_school
  )
  ...
  # Check that the two parameter lists are identical:
  expect_identical(
    object = parameters_leisure,
    expected = test_parameters_leisure
  )
  ...
  # Check that the two parameter lists are identical:
  expect_identical(
    object = parameters_household,
    expected = test_parameters_household
  )
})
...
test_generate_far_uvc_switches <- function(setting, target, type, tol = 1) {
  ...
  testthat::expect_vector(
    x[[paste0("uvc_", setting)]],
    ptype = double(),
    size = length(x$setting_sizes[[setting]])
  )
  testthat::expect_equal(
    x$human_population * coverage,
    sum(x$setting_sizes[[setting]] * x[[paste0("uvc_", setting)]]),
    tolerance = tol
  )
}
