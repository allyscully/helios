...
#' @export
set_setting_specific_riskiness <- function(
  parameters_list,
  setting,
  mean,
  sd,
  ...
  if (
    ...
    stop(
      "Error: Input setting invalid - far UVC only deployable in workplace, school, leisure, or household settings"
    )
  }
...
#' @family setting-specific riskiness
#' @export
generate_setting_specific_riskinesses <- function(
  parameters_list,
  setting,
  number_of_locations
) {
  ...
  if (parameters_list[[paste0("setting_specific_riskiness_", setting)]]) {
    setting_specific_riskiness <- EnvStats::rlnormTrunc(
      ...
      meanlog = parameters_list[[paste0(
        "setting_specific_riskiness_",
        setting,
        "_meanlog"
      )]],
      sdlog = parameters_list[[paste0(
        "setting_specific_riskiness_",
        setting,
        "_sdlog"
      )]],
      min = parameters_list[[paste0(
        "setting_specific_riskiness_",
        setting,
        "_min"
      )]],
      max = parameters_list[[paste0(
        "setting_specific_riskiness_",
        setting,
        "_max"
      )]]
