...
school_matrix <- array(
  dim = c(
    length(school_ACPH_range),
    length(infection_prevalence),
    length(time_in_room)
  )
)
dimnames(school_matrix) <- list(
  airchanges_per_hour = school_ACPH_range,
...
for (i in 1:length(school_ACPH_range)) {
  for (j in 1:length(infection_prevalence)) {
    ...
    Css <- ((pi * I) / (alpha * room_vol)) *
      (1 - exp(-((alpha * room_vol) * time_in_room / room_vol)))
...
school_melt_df <- melt(
  school_matrix,
  varnames = c("airchanges_per_hour", "infection_prevalence", "time"),
  value.name = "prob_inf"
)
...
school_melt_df <- school_melt_df %>%
  mutate(
    ACPH = case_when(
      airchanges_per_hour == school_ACPH_range[1] ~ "ACPH Min",
...
workplace_matrix <- array(
  dim = c(
    length(workplace_ACPH_range),
    length(infection_prevalence),
    length(time_in_room)
  )
)
dimnames(workplace_matrix) <- list(
  airchanges_per_hour = workplace_ACPH_range,
  infection_prevalence = paste0(100 * infection_prevalence, "% Prev."),
  time = paste0(time_in_room)
)
...
workplace_melt_df <- workplace_melt_df %>%
  mutate(
    ACPH = case_when(
      airchanges_per_hour == workplace_ACPH_range[1] ~ "ACPH Min",
...
leisure_matrix <- array(
  dim = c(
    length(leisure_ACPH_range),
    length(infection_prevalence),
    length(time_in_room)
  )
)
dimnames(leisure_matrix) <- list(
  airchanges_per_hour = leisure_ACPH_range,
  infection_prevalence = paste0(100 * infection_prevalence, "% Prev."),
  time = paste0(time_in_room)
)
...
leisure_melt_df <- leisure_melt_df %>%
  mutate(
    ACPH = case_when(
      airchanges_per_hour == leisure_ACPH_range[1] ~ "ACPH Min",
...
household_matrix <- array(
  dim = c(
    length(household_ACPH_range),
    length(infection_prevalence),
    length(time_in_room)
  )
)
dimnames(household_matrix) <- list(
  airchanges_per_hour = household_ACPH_range,
  infection_prevalence = paste0(100 * infection_prevalence, "% Prev."),
  time = paste0(time_in_room)
)
...
household_melt_df <- household_melt_df %>%
  mutate(
    ACPH = case_when(
      airchanges_per_hour == household_ACPH_range[1] ~ "ACPH Min",
...
saveRDS(
  overall_df,
  file = "inst/blueprint_july_transient_wells_riley_infection_risk_variation.rds"
)

a <- ggplot(
  subset(overall_df, infection_prevalence == "1% Prev."),
...
overall_df2 <- overall_df %>%
  ...
  summarise(
    ratio = prob_inf[airchanges_per_hour == min(airchanges_per_hour)] /
      prob_inf[airchanges_per_hour == max(airchanges_per_hour)]
  )

b <- ggplot(
  overall_df2,
  aes(x = as.numeric(time), y = ratio, col = setting_type)
) +
  geom_line() +
  geom_point() +
