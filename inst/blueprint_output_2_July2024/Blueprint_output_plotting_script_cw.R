...
simulations_to_run <- readRDS(
  "inst/blueprint_output_2_July2024/simulations_to_run.rds"
)
parameter_lists <- readRDS(
  "inst/blueprint_output_2_July2024/parameter_lists.rds"
)
...
for (i in 1:length(outputs)) {
  ...
  outputs_processed[[i]]$new_timestep <- outputs_processed[[i]]$timestep -
    min(outputs_processed[[i]]$timestep)
}
...
ggplot(
  ...
  geom_line(alpha = 1) +
  geom_vline(
    xintercept = c(0, 365, 730, 1095, 1460),
    linewidth = 0.25,
    linetype = "dashed"
  ) +
...
ggplot(overall2, aes(x = daily_timestep, col = archetype)) +
  ...
  geom_ribbon(
    aes(
      ...
      ymax = 1000 * E_new_upper / population
    ),
    alpha = 0.15
  ) +
  geom_vline(
...
overall3 <- overall %>%
  ...
  summarise(
    incidence_pre_uvc = sum(E_new[1:index]) / 4,
    incidence_after_uvc = sum(E_new[(index + 1):end]) / 2
  ) %>%
  pivot_longer(
    cols = c(incidence_pre_uvc, incidence_after_uvc),
    values_to = "incidence",
    names_to = "period"
  )

overall3$period <- factor(
  overall3$period,
  levels = c("incidence_pre_uvc", "incidence_after_uvc")
)
ggplot(
  overall3,
  aes(x = 100 * efficacy, y = incidence * 1000 / population, fill = period)
) +
  geom_bar(stat = "identity", position = "dodge", col = "black") +
  scale_fill_manual(
    values = c("#E1E1E1", "#324A5F"),
    labels = c("Incidence\nPre-UVC", "Incidence\nPost-UVC"),
...
overall4 <- overall %>%
  ...
  mutate(
    incidence_reduction = incidence_pre_uvc - incidence_after_uvc,
    incidence_percentage_reduction = incidence_reduction / incidence_pre_uvc
  )
ggplot(
  overall4,
  aes(x = 100 * efficacy, y = 100 * incidence_percentage_reduction)
) +
  geom_bar(
    stat = "identity",
    position = "dodge",
...
overall5 <- overall %>%
  ...
  group_by(archetype, efficacy, coverage) %>%
  summarise(
    incidence_percentage_reduction_mean = mean(incidence_percentage_reduction),
...
ggplot(
  overall5,
  aes(x = factor(100 * efficacy), y = 100 * incidence_percentage_reduction_mean)
) +
  geom_bar(
    stat = "identity",
    position = "dodge",
...
for (i in 1:40) {
  ...
  plot(
    x$timestep * 0.5,
    x$n_external_infections,
    ylim = c(0, max(x$n_external_infections)),
    type = "l",
    col = "#F15A24",
    xlab = "Time",
    ylab = "I Count"
