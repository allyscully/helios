...
### Disease states
health_var <- CategoricalVariable$new(
  categories = c("S", "E", "I", "R"),
  initial_values = c(
    rep("S", N - initial_infections),
    rep("E", initial_infections)
  )
)

### Age categories
age_var <- CategoricalVariable$new(
  categories = c("child", "adult", "elderly"),
...
workplace <- sample(
  ...
  replace = TRUE,
  prob = NULL
) # randomly sample workplaces for each adult
...
workplace_var <- CategoricalVariable$new(
  categories = as.character(0:10),
  initial_values = workplaces
...
school <- sample(
  as.character(1:10),
  size = length(index_child),
  replace = TRUE,
  prob = NULL
) # randomly sample schools for each child
...
schools_var <- CategoricalVariable$new(
  categories = as.character(0:10),
  initial_values = schools
) # create categorical variable
...
households_var <- CategoricalVariable$new(
  categories = as.character(1:1000),
  initial_values = household
) # create categorical variable
...
exposure_process <- function(t) {
  ...
  for (i in 1:1000) {
    ...
    spec_household_FOI <- beta_household *
      spec_household_I$size() /
      spec_household$size() ## this calculation needs more in it
  ...
  for (i in 1:10) {
    ...
    spec_workplace_FOI <- beta_workplace *
      spec_workplace_I$size() /
      spec_workplace$size() ## this calculation needs more in it
...
matplot(
  ...
  lwd = 2,
  lty = 1,
  col = adjustcolor(col = health_cols, alpha.f = 0.85),
  xlab = "Time",
  ylab = "Count"
)
legend(
