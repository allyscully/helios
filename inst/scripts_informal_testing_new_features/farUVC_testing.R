...
nofarUVC_parameters_list <- get_parameters(
  overrides = list(
    beta_household = 0.2,
    beta_workplace = 0.2,
    beta_school = 0.2,
...
matplot(
  x = nofarUVC_output[[1]] * nofarUVC_parameters_list$dt,
  y = nofarUVC_output[-1],
  type = "l",
  lwd = 2,
  lty = 1,
...
farUVC_parameters_list <- get_parameters(
  overrides = list(
    beta_household = 0.1,
    beta_workplace = 0.1,
    beta_school = 0.1,
...
matplot(
  x = farUVC_output[[1]] * farUVC_parameters_list$dt,
  y = farUVC_output[-c(1)],
  type = "l",
  lwd = 2,
  lty = 1,
...
plot(
  nofarUVC_output[[1]] * nofarUVC_parameters_list$dt,
  nofarUVC_output[, 3],
  type = "l"
)
lines(
  farUVC_output[[1]] * farUVC_parameters_list$dt,
  farUVC_output[, 3],
  col = "red"
)
...
par(mfrow = c(1, 2))
source("R/model.R")
epidemic_parameters_list <- get_parameters(
  overrides = list(
...
health_cols <- c("royalblue3", "firebrick3", "darkorchid3", "orange2")
matplot(
  x = x[[1]] * epidemic_parameters_list$dt,
  y = x[-1],
  type = "l",
  lwd = 2,
  lty = 1,
...
endemic_parameters_list <- get_parameters(
  overrides = list(
...
health_cols <- c("royalblue3", "firebrick3", "darkorchid3", "orange2")
matplot(
  x = y[[1]] * endemic_parameters_list$dt,
  y = y[-1],
  type = "l",
  lwd = 2,
  lty = 1,
