...
baseline_household_demographics <- read_csv(
  "data-raw/Hinch_et_al_baseline_household_demographics.csv"
)
usethis::use_data(baseline_household_demographics)

schools_england <- read_csv(
  "data-raw/spc_school_level_underlying_data_23112023.csv"
)
...
workplaces_large_data <- read.csv(
  "data-raw/us_naicssector_large_emplsize_2019.csv"
) %>%
  ...
  filter(
    Enterprise.Size %in%
      c(
        "09: 5,000-9,999 employees",
        "10: 10,000-19,999 employees",
        "11: 20,000+ employees"
      )
  )
