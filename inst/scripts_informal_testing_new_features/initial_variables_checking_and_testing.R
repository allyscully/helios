...
parameters <- list(
  ...
  N = 100,
  age_group_proportions = c(0.1, 0.6, 0.1),
  num_workplaces = 5,
  num_schools = 2,
  seed = 20,
...
test_age_class_variable <- individual::CategoricalVariable$new(
...
hist(
  truncdist::rtrunc(
    ...
    a = 0,
    b = Inf,
    lambda = parameters$mean_household_size
  ),
  breaks = 20
)

table(table(households)) / max(households)
table(truncdist::rtrunc(
  n = N,
  spec = "pois",
  a = 0,
  b = Inf,
