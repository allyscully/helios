...
setwd(
  "./inst/blueprint_output_3_Sep9/Report_3_Endemic/Endemic_Simulation_Batch_3/"
)
...
for (i in 1:nrow(simulations_to_run)) {
  ...
  timestep_uvc_on <- round(
    ((years_to_simulate - years_uvc_on_for) * 365) / parameter_lists[[i]]$dt
  )
...
saveRDS(
  parameter_lists,
  file = "./endemic_simulations_parameter_lists_batch_3.rds"
)
...
for (i in 1:length(lower_bounds)) {
  ...
  assign(
    x = paste0(
      "endemic_simulation_parameter_list_",
      lower_bounds[i],
      "_",
      upper_bounds[i]
    ),
  ...
  saveRDS(
    ...
    file = paste0(
      "./endemic_batch_3_inputs/endemic_simulation_parameter_list_",
      i,
      "_",
      lower_bounds[i],
      "_",
