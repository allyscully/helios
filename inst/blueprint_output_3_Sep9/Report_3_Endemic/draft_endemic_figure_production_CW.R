...
indices_pre_UVC <- (timestep_uvc_on -
  (365 * 4 / temp_obj[[1]]$parameters$dt)):timestep_uvc_on
indices_post_UVC <- (timestep_uvc_on + 1):(365 *
  years_to_simulate /
  temp_obj[[1]]$parameters$dt)
...
for (i in 1:length(output_files)) {
  ...
  tempdf <- bind_rows(tempdf) %>%
    ...
    summarise(
      avg_inc_pre_UVC = sum(E_new[indices_pre_UVC]) / years_pre_UVC,
...
saveRDS(
  endemic_summary_output,
  file = "inst/blueprint_output_3_Sep9/Report_3_Endemic/Endemic_Simulation_Batch_3/endemic_summary_outputs.rds"
)

avg_summary_outputs <- summary_outputs %>%
  group_by(archetype, coverage_type, coverage, efficacy, disease_status) %>%
  summarise(
    mean_reduction_incidence = mean(reduction_incidence),
...
inc_reduction_plot <- ggplot(
  ...
  geom_errorbar(
    aes(
      ymin = 100 * lower_reduction_incidence,
...
# Plot
inc_reduction_alt_plot <- ggplot(
  data = subset(avg_summary_outputs, coverage <= 0.7),
  aes(
    x = 100 * coverage,
    y = 100 * mean_reduction_incidence,
    fill = coverage_type
  )
) +
  geom_bar(
...
avg_summary_outputs_diff <- summary_outputs %>%
  ...
  pivot_wider(
    id_cols = c(iteration, archetype, coverage, efficacy, disease_status),
  ...
  summarise(
    mean_diff_rel = median(diff_rel, na.rm = TRUE),
    lower_diff_rel = quantile(diff_rel, 0.25, na.rm = TRUE),
    upper_diff_rel = quantile(diff_rel, 0.75, na.rm = TRUE)
  )

extra_impact_rel <- ggplot(
  data = subset(avg_summary_outputs_diff, coverage <= 0.9),
...
for_plot <- subset(temp_df[[2]], timestep >= 10730) %>%
  mutate(
    time_period = ifelse(
      ((timestep - 10730) * temp_df$parameters$dt) / 365 > 5,
      "Pre-UVC",
      "Post-UVC"
    )
  )
for_plot_avg <- for_plot %>%
  ...
  summarise(
    time_period_avg = mean(1000 * E_new / 100000),
...
exemplar_plot <- ggplot() +
  geom_line(
    data = for_plot,
    aes(
      x = ((timestep - 10730) * temp_df$parameters$dt) / 365,
      y = 1000 * E_new / 100000,
      col = ifelse(
        ((timestep - 10730) * temp_df$parameters$dt) / 365 > 5,
        "Pre-UVC",
  ...
  scale_colour_manual(
    values = c("lightgrey", "orange"),
    labels = c("Pre-UVC", "Post-UVC"),
    name = "Time Period"
  ) +
  geom_segment(
    data = for_plot_avg,
    aes(x = start, xend = end, y = time_period_avg, yend = time_period_avg),
    linetype = "dashed",
    color = "black"
  ) +
  theme_bw() +
  labs(
    x = "Year (far UVC on at Year 5)",
    y = "Daily Incidence Per 1,000 Population"
  )
