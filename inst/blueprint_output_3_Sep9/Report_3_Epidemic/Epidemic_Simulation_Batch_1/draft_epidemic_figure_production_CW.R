...
output_files <- list.files(
  "./Report_3_Epidemic/Epidemic_Simulation_Batch_1/epidemic_batch_1_outputs/"
)
...
for (i in 1:length(output_files)) {
  ...
  tempdf2 <- bind_rows(tempdf) |>
    mutate(
      coverage_type = ifelse(is.na(coverage_type), "none", coverage_type)
    ) |>
...
avg_summary_outputs <- summary_outputs |>
  ...
  summarise(
    mean_peak_timing = mean(peak_timing),
...
# Save the dataframes:
saveRDS(
  object = summary_outputs,
  file = "./Report_3_Epidemic/Epidemic_Simulation_Batch_1/epidemic_summary_outputs.rds"
)
saveRDS(
  object = avg_summary_outputs,
  file = "./Report_3_Epidemic/Epidemic_Simulation_Batch_1/epidemic_avg_summary_outputs.rds"
)
...
ggplot() +
  geom_line(
    data = subset(
      avg_summary_outputs,
      coverage_type == "random" & coverage <= 0.7
    ),
    aes(x = 100 * coverage, y = mean_final_size / 1000),
    col = blueprint_colours[1],
    linewidth = 1
  ) +
  geom_line(
    data = subset(
      avg_summary_outputs,
      coverage_type == "targeted_riskiness" & coverage <= 0.7
    ),
    aes(x = 100 * coverage, y = mean_final_size / 1000),
    col = blueprint_colours[2],
    linewidth = 1
  ) +
...
avg_summary_outputs |>
  ...
  ggplot(aes(
    x = 100 * coverage,
    y = mean_final_size / 1000,
    colour = coverage_type
  )) +
  geom_line(linewidth = 1.2) +
  scale_colour_manual(
    values = blueprint_colours,
  ...
  geom_ribbon(
    aes(
      ymin = min_final_size / 1000,
      ymax = max_final_size / 1000,
      fill = coverage_type
    ),
    colour = NA,
    alpha = 0.3,
...
ggplot() +
  geom_line(
    ...
    aes(x = 100 * coverage, y = mean_peak / 1000),
    col = blueprint_colours[1],
    linewidth = 1
  ) +
  geom_line(
...
avg_summary_outputs |>
  ...
  geom_ribbon(
    aes(ymin = min_peak / 1000, ymax = max_peak / 1000, fill = coverage_type),
    colour = NA,
    alpha = 0.3,
    show.legend = FALSE
  ) +
...
ggplot() +
  geom_line(
    ...
    aes(x = 100 * coverage, y = mean_peak_timing),
    col = blueprint_colours[1],
    linewidth = 1
  ) +
  geom_line(
...
avg_summary_outputs |>
  ...
  geom_ribbon(
    aes(ymin = min_peak_timing, ymax = max_peak_timing, fill = coverage_type),
    colour = NA,
    alpha = 0.3,
    show.legend = FALSE
  ) +
...
epidemic_parameter_table <- readRDS(
  "./Report_3_Epidemic/Epidemic_Simulation_Batch_1/epidemic_simulations_table_batch_1.rds"
)
...
epidemic_parameter_table |>
  ...
  select(ID) -> batch_of_interest
min(batch_of_interest)
max(batch_of_interest)
...
str_sort(
  list.files(
    "./Report_3_Epidemic/Epidemic_Simulation_Batch_1/epidemic_batch_1_inputs/"
  ),
  numeric = TRUE
)

##' The IDs are split across two batches, 27 and 28, so lets load the 27th:
exemplar_batch <- readRDS(
  "./Report_3_Epidemic/Epidemic_Simulation_Batch_1/epidemic_batch_1_outputs/scenario_output_batch_1_27.rds"
)
...
exemplar_simulation |>
  filter(timestep <= 365) |>
  ggplot(aes(x = timestep, y = 1000 * E_new / 100000)) +
