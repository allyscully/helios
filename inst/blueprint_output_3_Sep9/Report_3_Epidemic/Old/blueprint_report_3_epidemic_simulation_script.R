...
saveRDS(
  object = results1,
  file = "./inst/blueprint_output_3_Sep9/Report_3_Epidemic/Report3_EpidemicSimulation_Outputs/test_epidemic_outputs.rds"
)
Sys.sleep(15)
...
ggplot(
  proc_outputs3,
  aes(x = coverage, y = final_size, colour = coverage_type)
) +
  geom_line() +
...
ggplot(
  proc_outputs3,
  aes(x = coverage, y = peak_timing, colour = coverage_type)
) +
  geom_line() +
  facet_grid(archetype ~ efficacy)
...
proc_outputs3 <- proc_outputs2 %>%
  ...
  summarise(peak_timing = mean(peak_timing), final_size = mean(final_size))
head(proc_outputs3)
ggplot(
  proc_outputs3,
  aes(x = coverage, y = peak_timing, colour = coverage_type)
