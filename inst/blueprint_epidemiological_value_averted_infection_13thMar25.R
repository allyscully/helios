...
df_comparison <- endemic_summary_outputs %>%
  ...
  mutate(Reff = ifelse(archetype == "flu", 1.5 * (1 - hypothetical_reduction), 2.5 *(1 - hypothetical_reduction)))
...
ggplot(data = df_comparison, aes(x = 100 * hypothetical_reduction,
  ...
  geom_errorbar(aes(ymin = 100 * incidence_percentage_reduction_lower,
                    ...
                    col = coverage), linewidth = 1) +
  geom_abline(slope = 1, linetype = "dashed") +
  facet_grid(factor(efficacy) ~ archetype) +
  labs(x = "Reduction in Annual Incidence\n Predicted By Simple Model",
       y = "Reduction in Annual Incidence\nPredicted by helios (%)") +
  theme_bw()

b <- ggplot(data = df_comparison, aes(x = Reff,
                                 y = 100 * incidence_percentage_reduction_mean / (100 * hypothetical_reduction),
                                 col = hypothetical_reduction)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = 100 * incidence_percentage_reduction_lower / (100 * hypothetical_reduction),
                    ymax = 100 * incidence_percentage_reduction_upper / (100 * hypothetical_reduction),
                    col = hypothetical_reduction), linewidth = 1) +
  facet_grid(. ~ archetype, scales = "free") +
  coord_cartesian(xlim = c(1, NA)) +
  labs(x = "(very) approximate reproduction number",
       y = "Epidemiological Value of Averted Infection",
       col = "Prop reduction\nin incidence\npredicted by\nsimple model") +
  theme_bw()

a <- ggplot(data = df_comparison, aes(x = 1- hypothetical_reduction,
                                 y = 100 * incidence_percentage_reduction_mean / (100 * hypothetical_reduction),
                                 col = hypothetical_reduction)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = 100 * incidence_percentage_reduction_lower / (100 * hypothetical_reduction),
  ...
  labs(x = "Proportion of Transmission Remaining Predicted By Simple Model\n(1 - Prop Reduction Predicted by Simple Model)",
       y = "Epidemiological Value of Averted Infection",
       col = "Prop reduction\nin incidence\npredicted by\nsimple model") +
  theme_bw()
